...
| [`k8s-crd-clusterapi.md`](k8s-crd-clusterapi.md) | CRD, Cluster API, VKS(vSphere Kubernetes Service) 교육 |
...
| [`podsecurity_admission.md`](podsecurity_admission.md) | Pod Security Admission(PSA) — 네임스페이스 단위 Pod 보안 정책 적용 방법 및 실습 가이드 |
...
```
╔══════════════════════════════════════════════════════════════╗
║           Kubernetes Addon Installer                        ║
║     자주 쓰는 오픈소스를 인터랙티브하게 설치합니다           ║
╚══════════════════════════════════════════════════════════════╝

  ── 개별 애드온 설치 ──────────────────────────────────────
    1) Prometheus             — 메트릭 수집 & 알림 (kube-prometheus-stack)
    2) Grafana                — 대시보드 시각화
    3) Velero + MinIO         — 클러스터 백업 & 복구
    4) Istio                  — 서비스 메시 (트래픽 관리, mTLS)
    5) Elasticsearch + Kibana — 로그 저장 & 검색 시각화 (ECK)
    6) Fluent Bit             — 경량 로그 수집 DaemonSet
    7) Kyverno               — K8s-Native 정책 엔진
    8) OPA Gatekeeper        — Rego 기반 정책 접근 제어
    9) NGINX Ingress         — L7 HTTP(S) 라우팅
   10) Contour               — Envoy 기반 Ingress Controller
   11) Loki                  — 경량 로그 집계 (Grafana)
   12) cert-manager          — TLS 인증서 자동 발급·갱신
   13) ArgoCD                — GitOps CD 플랫폼
   14) FluxCD                — GitOps Toolkit
   15) Jenkins               — CI/CD 자동화 서버

  ── 관리 ──────────────────────────────────────────────────
    s) 설치 현황 확인        — 전체 애드온 설치 상태 조회
    d) 애드온 삭제            — 설치된 애드온 선택 삭제
    q) 종료
...
| 메뉴 | 설명 |
|------|------|
| **`s` — 설치 현황 확인** | 15종 전체 애드온의 설치 상태를 테이블로 조회 (✔ 설치됨 / ⚠ 부분 실행 / ✗ 미설치) |
